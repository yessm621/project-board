    ...
    public void updateArticle(Long articleId, ArticleDto dto) {
        try {
            ...
            if (article.getUserAccount().getUserId().equals(userAccount.getUserId())) {
                ...
                if (dto.content() != null) {
                    article.setContent(dto.content()); // java record는 getContent()가 아닌 content()로 작성한다.
                }
                article.setHashtag(dto.hashtag());
            }
        } catch (EntityNotFoundException e) {
            log.warn("게시글 업데이트 실패. 게시글을 수정하는데 필요한 정보를 찾을 수 없습니다. - {}", e.getLocalizedMessage()); // 인터폴레이션 방식
        }
    }
